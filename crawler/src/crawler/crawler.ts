...
export class Crawler {
  ...
  private getTotalCrawledCount() {
    return this.crawledPages.length + this.pagesToCrawl.length;
  }
  ...
  private async createExecution(page: CrowledWebPage): Promise<void> {
    ...
    let status =
      page.status === "done" ? ExecutionStatus.SUCCESS : ExecutionStatus.FAILED;
    if (page.status == "notValid") {
      status = ExecutionStatus.NOTVALID;
    }
      ...
      if (!regex.test(this.currentCrawlingWebPage.url)) {
        console.log(
          `(Crawler ${threadId}) Dont match to regex (${task.regex}): ${this.currentCrawlingWebPage.url}, ` +
            ...
            }/${this.getTotalCrawledCount()}`
        );
        this.currentCrawlingWebPage.crawlTimeEnd = Date.now();
        this.currentCrawlingWebPage.status = "notValid";
      ...
      try {
        ...
        if (html) {
          ...
          for (const link of this.currentCrawlingWebPage.links) {
            if (
              !this.seenPages.has(link) &&
              !this.pagesToCrawl.some((page) => page.url === link)
            ) {
        ...
        console.log(
          `(Crawler ${threadId}) Crawling failed ${this.currentCrawlingWebPage.url}, ` +
            `current progress: ${
              this.crawledPages.length
            }/${this.getTotalCrawledCount()}, ${error}`
        );
      } finally {
        this.currentCrawlingWebPage.crawlTimeEnd = Date.now();
        await this.createExecution(this.currentCrawlingWebPage);
        ...
        const crawlTime =
          this.currentCrawlingWebPage.crawlTimeEnd -
          this.currentCrawlingWebPage.crawlTimeStart;
        // console.log(`(crawler ${threadId}) Time spent crawling ${this.currentCrawlingWebPage.url}: ${crawlTime} ms`);
      }
    }

    console.log(
      `(Crawler ${threadId}) Crawling finished, total pages crawled: ${this.crawledPages.length}`
    );
